...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($error)) {
        ...
        if (isset($_FILES['photo']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
            ...
            else { $photo = uploadPhoto($_FILES['photo']); }
        }
    }
    if (!isset($error)) {
        $data = [
        ...
        try {
            $fields = 'student_code, first_name, last_name, father_name, mother_name, gender, date_of_birth, phone, email, address, enrollment_date, notes, photo, created_by';
            $placeholders = '?,?,?,?,?,?,?,?,?,?,?,?,?,?';
            insert("INSERT INTO students ($fields) VALUES ($placeholders)", array_values($data));
