...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    update("UPDATE courses SET course_code=?, course_name=?, description=?, teacher_id=?, credit_hours=?, fee=?, max_students=? WHERE id=?", [
        trim($_POST['course_code']), trim($_POST['course_name']), trim($_POST['description']),
        $_POST['teacher_id'] ?: null, $_POST['credit_hours'], $_POST['fee'], $_POST['max_students'], $id
    ]);
